...
export function PayrollSecurityDialog({ isOpen, onUnlock }: PayrollSecurityDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!success) {
        setErrorMsg("Mật khẩu bảo mật không chính xác (Gợi ý: 0111)");
      }
    } catch (err: any) {
      ...
      <DialogContent className="sm:max-w-[420px] p-6 rounded-2xl border-border bg-card shadow-2xl animate-in zoom-in-95 duration-200">
        <DialogHeader className="text-center space-y-2 flex flex-col items-center">
          ...
          <DialogDescription className="text-xs text-muted-foreground text-center">
            Nhập mật khẩu mở khóa được quy định trong môi trường hệ thống để xem dữ liệu Bảng Công & Thù Lao Giảng Viên.
          </DialogDescription>
        </DialogHeader>
