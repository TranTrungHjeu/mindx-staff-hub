...
export function Sidebar({ currentView, onViewChange }: SidebarProps) {
  ...
  return (
    <>
      ...
      <div className="flex md:hidden h-14 w-full bg-card/90 backdrop-blur-md border-b border-border px-3 items-center justify-between shrink-0 z-30">
        <div className="flex items-center gap-2">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => setIsMobileOpen(true)}
            className="h-9 w-9 rounded-xl text-foreground hover:bg-muted active:scale-95 transition-all"
          ...
          <span className="font-bold text-xs tracking-tight text-foreground">
            {currentView === "schedules" ? "Ma Trận Lịch Tuần" : "Bảng Công Giảng Viên"}
          </span>
        </div>

        {/* Quick View Switcher Pills */}
        <div className="flex items-center p-0.5 rounded-xl bg-muted/70 border border-border/80">
          <button
            type="button"
            onClick={() => onViewChange("schedules")}
            className={`px-2.5 py-1 rounded-lg text-[11px] font-bold transition-all ${
              currentView === "schedules"
                ? "bg-[#000056] text-white shadow-2xs"
      ...
      {isMobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden flex">
          ...
          <div className="relative w-4/5 max-w-xs bg-card border-r border-border h-[100dvh] flex flex-col justify-between shadow-2xl z-10 animate-in slide-in-from-left duration-300">
            <div className="flex flex-col">
              {/* Drawer Header */}
              <div className="p-3 border-b border-border flex items-center justify-end bg-slate-900 text-white">
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => setIsMobileOpen(false)}
                ...
                {navItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => !item.disabled && handleNavClick(item.id as ViewMode)}
                      className={`w-full h-11 flex items-center px-3 rounded-xl text-xs font-bold transition-all relative ${
                        item.disabled
                    ...
                    >
                      <Icon className="h-4.5 w-4.5 shrink-0 mr-3" />
      ...
      <aside
        className={`hidden md:flex h-[100dvh] bg-card border-r border-border flex-col justify-between transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] z-40 shrink-0 select-none ${
